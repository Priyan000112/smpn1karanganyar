...
export default function VisiMisiPage() {
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      {/* Visi */}
      <div className="bg-primary-600 rounded-2xl p-8 mb-8 text-white">
        ...
        <ol className="space-y-4">
          {misiList.map((misi, i) => (
            <li key={i} className="flex gap-4">
              <span className="flex-shrink-0 w-8 h-8 bg-primary-100 dark:bg-primary-900/40 text-primary-700 dark:text-primary-400 rounded-full flex items-center justify-center text-sm font-bold">
                {i + 1}
              </span>
              <p className="text-slate-600 dark:text-slate-400 leading-relaxed pt-1">{misi}</p>
            </li>
          ))}
        </ol>
      </div>
      ...
      <div className="mt-8 bg-amber-50 dark:bg-amber-900/20 rounded-2xl p-6 border border-amber-200 dark:border-amber-800">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-xl font-bold text-amber-800 dark:text-amber-300">Tujuan Sekolah</h2>
        </div>
